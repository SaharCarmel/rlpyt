...
class SerialEvalCollector(BaseEvalCollector):
    ...
    def dump_video(self, itr):
        traj_infos = [self.TrajInfoCls() for _ in range(len(self.dump_env))]
        completed_traj_infos = list()
        observations = list()
        for env in self.dump_env:
            observations.append(env.reset())
        observation = buffer_from_example(observations[0], len(self.dump_env))
        for b, o in enumerate(observations):
            observation[b] = o
        # action = buffer_from_example(self.dump_env[0].action_space.null_value(),
        #     len(self.dump_env))
        action = buffer_from_example(0,
            len(self.dump_env))
        reward = np.zeros(len(self.dump_env), dtype="float32")
        obs_pyt, act_pyt, rew_pyt = torchify_buffer((observation, action, reward))
        self.agent.reset()
        self.agent.eval_mode(itr)
        for t in range(self.max_T):
            act_pyt, agent_info = self.agent.step(obs_pyt, act_pyt, rew_pyt)
            action = numpify_buffer(act_pyt)
            for b, env in enumerate(self.dump_env):
                o, r, d, env_info = env.step(action[b])
                traj_infos[b].step(observation[b], action[b], r, d,
                    agent_info[b], env_info)
                if getattr(env_info, "traj_done", d):
